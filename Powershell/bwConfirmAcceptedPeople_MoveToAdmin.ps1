...
ForEach ($member_id in $org_members.trim('"')) {
    ...
    Write-Host "Upgrading to admin: $member_id"
    $member_data = (Invoke-RestMethod -Method GET -Uri $api_url/public/members/$member_id -Headers $headers)
    $params = @{type=2;accessAll=$member_data.accessAll;externalId=$member_data.externalId;resetPasswordEnrolled=$member_data.resetPasswordEnrolled;collections=$member_data.collections} | ConvertTo-Json
