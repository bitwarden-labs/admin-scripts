...
if (bw --session "$session_key" list org-collections --organizationid $organization_id | jq -r '.[].name' | grep -x "$member"); then
...
memberid="$(echo $allorgmembers | jq -r -c --arg n "$member" '.data[] | select(.name==$n) | .id')"
...
collectionid="$(bw --session $session_key get template org-collection | jq -c --arg n "$member" --arg c "$organization_id" '.name=$n | .organizationId=$c | del(.groups[1])' | bw encode | bw --session $session_key create org-collection --organizationid $organization_id | jq -r '.id')"
newmemberbody=$(echo $origmemberbody | jq -r -c --arg newcol "$collectionid" --arg i "$memberid" 'select(.id==$i) | .collections += [{"id": $newcol, "readOnly": false, "hidePasswords": false, "manage": true}]')
