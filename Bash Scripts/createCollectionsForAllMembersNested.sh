...
for member in ${org_members[@]}; do
    ...
    if (echo $all_collections | jq -r '.[].name' | grep -x "$parentCollection/$member" > /dev/null); then
	...
	'.name="'$parentCollection'/" + $n | .organizationId=$c | del(.groups[1]) | del(.groups[0]) | del(.users[1]) | .users[0].id=$m | .users[0].manage=true' \
	| bw encode \
