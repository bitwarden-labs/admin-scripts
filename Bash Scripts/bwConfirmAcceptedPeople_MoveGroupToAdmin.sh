...
# create groups.txt. One group name each line. Only members in these groups will be upgraded to Admin
...
for member_id in ${org_members[@]} ; do
	...
	if [[ " ${group_member_list[*]} " == *" $member_id "* ]]; then
		echo "upgrading to admin: $member_id"
		...
		curl -X PUT "$api_url/public/members/$member_id" -H 'Content-Type: application/json' -H 'Authorization: Bearer '$bearer_token'' -d "{\"type\":2, \"accessAll\":$access_all, \"externalId\": \"$external_id\", \"resetPasswordEnrolled\":$reset_password_enrolled, \"collections\":$collections}"
	fi	
done
